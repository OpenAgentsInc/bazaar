...
export const ImmortalBrowserOperationSchema = Schema.Literals(
  IMMORTAL_BROWSER_OPERATIONS
)
export type ImmortalBrowserOperation =
  typeof ImmortalBrowserOperationSchema.Type
...
const ByteSchema = UInt16Schema.pipe(
  Schema.check(Schema.isLessThanOrEqualTo(255))
)
const ByteArraySchema = Schema.Array(ByteSchema)
const LowerHexSchema = Schema.String.check(
  Schema.isPattern(/^(?:[0-9a-f]{2})+$/)
)
const DecimalStringSchema = Schema.String.check(
...
export type ImmortalSignedRecordDelivery =
  typeof ImmortalSignedRecordDeliverySchema.Type

export const ImmortalSessionDeliveryInputSchema = Schema.Struct({
  raw_signed_event_hex: LowerHexSchema,
  observed_at: NonNegativeSafeIntegerSchema,
  provenance: Schema.Literals(["locally_signed", "direct"]),
})
export type ImmortalSessionDeliveryInput =
  typeof ImmortalSessionDeliveryInputSchema.Type
...
export type ImmortalRequesterSessionView =
  typeof ImmortalRequesterSessionViewSchema.Type
...
export type ImmortalSessionIngestResult =
  typeof ImmortalSessionIngestResultSchema.Type
...
export const ImmortalContractDraftSchema = Schema.Record(
  Schema.String,
  Schema.Json
)
...
export type ImmortalExitPackageInspection =
  typeof ImmortalExitPackageInspectionSchema.Type
...
export type ImmortalWasmSource =
  Response | ArrayBuffer | ArrayBufferView | URL | string
...
const invokeOperation: unique symbol = Symbol(
...
const makeClient = (
  ...
  Effect.gen(function* () {
    ...
    const invokeUnlocked = Effect.fn(
    ...
    )(function* (operation: ImmortalBrowserOperation, input: Schema.Json) {
      ...
      const parsed = yield* Effect.try({
        try: () =>
          ...
          abiError("response", "browser_response_invalid", cause),
      })
...
export const loadImmortalBrowserClient = Effect.fn(
  ...
  const instance = yield* Effect.tryPromise({
    try: () => WebAssembly.instantiate(module, {}),
      ...
      abiError("instantiate", "browser_wasm_instantiate_failed", cause),
  })
...
export const ingestRequesterSession = (
  ...
  invokeDecoded(
    ...
    "session_ingest",
    input,
    ImmortalSessionIngestResultSchema
  )
