...
export function normalizeOffering(event) {
    const base = {
    ...
    const profile = content && typeof content.mkt_swp === "object" && content.mkt_swp !== null
        ? content.mkt_swp
        : undefined;
    if (!profile) {
        return {
    ...
    const swapTypes = Array.isArray(profile.swap_types)
        ? profile.swap_types.filter((value) => typeof value === "string")
        : [];
    const sides = [];
    let parseError;
...
export function normalizeProviderProfile(event) {
    const content = contentRecord(event);
    const profile = content && typeof content.mkt_swp === "object" && content.mkt_swp !== null
        ? content.mkt_swp
        : undefined;
    const labelCandidate = profile &&
    ...
    return {
        ...
        status: tagValue(event.tags, "status"),
        label: typeof labelCandidate === "string"
            ? labelCandidate
