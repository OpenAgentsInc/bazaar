...
export async function joinNetwork(args) {
    ...
    try {
        ...
        return toolError("join_script_not_found", `${script} does not exist locally. The immortal join kit (immortal#45) publishes kind 39600 + 39601 as part of ` +
            "provider start; use spin_up_node once the kit is present in the immortal checkout.");
    }
    const hasPublishEntrypoint = PUBLISH_ENTRYPOINT_PATTERNS.some((pattern) => pattern.test(source));
    if (!hasPublishEntrypoint) {
        return toolError("no_publish_entrypoint", "The installed join-regtest.sh has no discrete publish entrypoint. It publishes kind 39600 (provider profile) " +
            ...
            "network_status as a discovered-tier provider.", { script });
    }
    const lines = [];
    return await new Promise((resolve) => {
        ...
        child.on("close", (code) => {
            ...
            if (code === 0) {
                ...
                resolve(toolError("publish_failed", `join-regtest.sh publish exited with code ${code}.`, { script, exitCode: code, outputLines: lines }));
            }
        });
    });
}
