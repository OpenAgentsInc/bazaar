...
import { assertHttpUrl, assertRegtestAddress, REGTEST_NETWORK, } from "../boundaries.js";
import { ok, toolError } from "../result.js";
export const FAUCET_REQUEST_SCHEMA = "openagents.immortal.public-regtest-faucet-request.v1";
...
export async function faucetFund(args) {
    assertRegtestAddress(args.address);
    const gateway = assertHttpUrl(args.gateway, "gateway");
    if (!Number.isSafeInteger(args.amountSat) ||
        args.amountSat < 1 ||
        args.amountSat > MAXIMUM_AMOUNT_SAT) {
        return toolError("amount_invalid", `amountSat must be an integer between 1 and ${MAXIMUM_AMOUNT_SAT} (regtest sats).`);
    }
    const endpoint = new URL(`${gateway.origin}${gateway.pathname.replace(/\/$/, "")}/v1/public-regtest/faucet`);
    let response;
    try {
        ...
        return toolError("faucet_unreachable", `Could not reach the gateway faucet at ${endpoint}: ${cause instanceof Error ? cause.message : String(cause)}`);
    }
    if (response.status === 404) {
        return toolError("faucet_unavailable", `The gateway at ${gateway.origin} does not expose the faucet capability yet ` +
    ...
    if (!response.ok) {
        return toolError("faucet_rejected", `The faucet rejected the request with HTTP ${response.status}.`, { response: body });
    }
    ...
    while (Date.now() < deadline) {
        ...
        try {
            ...
            lastStatus = pollBody;
            const status = typeof pollBody.status === "string" ? pollBody.status : undefined;
            if (status === "paid" || status === "confirmed") {
                return ok({
                    schema: "openagents.immortal-mcp.faucet-fund.v1",
                    state: "paid",
    ...
    return toolError("faucet_poll_timeout", "The faucet did not report paid within 60 seconds. The request may still settle; poll the status URL directly.", { statusUrl, lastStatus });
}
