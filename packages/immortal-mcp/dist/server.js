...
const APPROVAL_NOTE = "EFFECTFUL: hosts should require explicit user approval before running this tool. ";
export function buildServer() {
    const server = new McpServer({ name: SERVER_NAME, version: SERVER_VERSION });
    server.registerTool("network_status", {
        title: "Immortal network status",
        description: "Read-only. Fetches the public regtest launch manifest envelope JSON, structure-checks it (envelope schema, " +
        ...
        inputSchema: {
            manifestUrl: z
                ...
                .max(2_048)
                .optional()
                .describe("URL serving the raw public regtest manifest envelope JSON (schema " +
    ...
    server.registerTool("list_offerings", {
        ...
        description: "Read-only. Takes one bounded EOSE-terminated REQ snapshot of kind 39601 offering heads per given relay " +
    ...
    server.registerTool("get_quotes", {
        ...
        description: "Read-only. V1 SCOPE CUT: always returns a typed not_implemented error. The no-spend RFQ flow requires the " +
    ...
    }, async (args) => toolError("not_implemented", "get_quotes is a v1 scope cut. The no-spend RFQ contract (competing signed quotes + selection policy) runs " +
        ...
        "quotes.", { requested: args }));
    server.registerTool("node_health", {
        title: "Local join-kit node health",
        description: "Read-only. Reports the local join-kit stack: `docker compose ps --format json` in the join directory " +
    ...
    server.registerTool("spin_up_node", {
        ...
        let progress = 0;
        const onLine = progressToken === undefined
            ? undefined
            : (line) => {
                progress += 1;
                void extra
    ...
    server.registerTool("join_network", {
    ...
    server.registerTool("faucet_fund", {
    ...
    server.registerTool("request_listing", {
