...
import { fetchManifestSummary, type ManifestSummary } from "../manifest.js"
...
export interface NetworkStatusArgs {
  manifestUrl?: string
}

export async function networkStatus(
  args: NetworkStatusArgs
): Promise<ToolResult> {
  ...
  try {
    ...
    return toolError(
      ...
      `Could not fetch or parse the manifest envelope: ${
  ...
  const offeringsByProvider = new Map<string, NormalizedOffering[]>()
  ...
  const relayIdsFor = (pubkey: string): string[] =>
    relayResults
      .filter((entry) =>
        entry.snapshot.events.some((event) => event.pubkey === pubkey)
      )
