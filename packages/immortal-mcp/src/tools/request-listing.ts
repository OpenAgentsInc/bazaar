...
import {
  ...
  assertHttpUrl,
  BoundaryError,
  rejectMainnetIdentifiers,
} from "../boundaries.js"
import { ok, type ToolResult } from "../result.js"

export interface RequestListingArgs {
  pubkey: string
...
export async function requestListing(
  ...
  if (
    new TextEncoder().encode(args.healthJson).byteLength >
  ...
  return ok({
    ...
    url: url.toString(),
    note:
      "Open this URL in a browser to file the pin request. This tool did not open a browser and did not " +
      "create the issue.",
  })
}
