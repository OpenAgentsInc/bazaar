...
export interface NodeHealthArgs {
  stateDir?: string
}
...
export async function nodeHealth(args: NodeHealthArgs): Promise<ToolResult> {
  ...
  try {
    ...
    return toolError(
      "join_kit_not_found",
      `No join-kit directory at ${joinDir}. The immortal join kit (immortal#45) is not installed there. ` +
  ...
  try {
    ...
    if (trimmed.startsWith("[")) {
      services = JSON.parse(trimmed) as unknown[]
  ...
  const healthFiles: Record<string, unknown> = {}
  for (const candidate of HEALTH_FILE_CANDIDATES) {
    ...
    try {
      ...
      if (raw.length > 64 * 1024) {
        ...
        try {
          healthFiles[candidate] = JSON.parse(raw) as unknown
        } catch {
  ...
  return ok({
    ...
    healthFiles:
      Object.keys(healthFiles).length > 0
        ? healthFiles
