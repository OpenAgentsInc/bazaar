...
export interface SpinUpNodeArgs {
  role: "provider" | "relay"
  relays?: string[]
  addnode?: string
  gateway?: string
...
export function defaultImmortalDir(): string {
...
export async function spinUpNode(
  ...
  try {
    ...
    return toolError(
      ...
      `${script} does not exist. The immortal join kit (immortal#45) is not present in this checkout. ` +
